...
class OmniGen2TrainDataset(torch.utils.data.Dataset):
    ...
    def process_item(self, data_item):
        assert data_item['captions'] is not None
        ...
        if drop_prompt:
            ...
            instruction = self.apply_chat_template(data_item['captions'][0], self.SYSTEM_PROMPT)
        ...
        if not drop_ref_img and 'input_images' in data_item and data_item['input_images'] is not None:
        ...
        elif "original_image" in data_item and data_item["original_image"] is not None:
            root_path = "/home/ma-user/work/yanli/test_yl/gRefCOCO" #"/home/ma-user/work/yanli/test_yl/Datasets"

            input_images_path = [os.path.join(root_path, data_item["original_image"])]
            ...
            #input_images[0] = self.image_processor.preprocess(input_images[0], max_pixels=max_input_pixels, max_side_length=self.max_side_length)
            output_image_path = os.path.join(root_path, data_item["segmented_image"])
        else:
