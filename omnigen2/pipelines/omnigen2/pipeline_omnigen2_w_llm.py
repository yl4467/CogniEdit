...
def apply_format(instruction):
    ...
    You are a helpful assistant that describes the images.
    <|im_end|>
    <|im_start|>user<|vision_start|><|image_pad|><|vision_end|>
    First describe what you see in this image and figure out how to edit the image based on: {instruction} <|im_end|>
    """
    return prompt
...
class OmniGen2Pipeline(DiffusionPipeline, OmniGen2LoraLoaderMixin):
    ...
    def _get_qwen2_prompt_embeds(
        ...
        prompt_caption, vision_input = self._apply_chat_template(prompt, images[0])
        prompt = [apply_format(prompt)] if isinstance(prompt, str) else prompt
        prompt = [apply_format(content) for content in prompt]
        print(prompt)
        ...
        #     return_tensors="pt",
        # )
        #image_inputs, video_inputs = process_vision_info(prompt_caption)
        enc_inputs = self.processor(images=vision_input, text=[prompt_caption],
                                    return_tensors="pt", padding=True).to(device)

        generated_ids = self.mllm.generate(**enc_inputs, max_new_tokens=128)
        generated_ids_trimmed = [
            out_ids[len(in_ids) :] for in_ids, out_ids in zip(enc_inputs.input_ids, generated_ids)
        ]
        ...
        enc_inputs1 = self.processor(images=images, text=prompt,
                                    return_tensors="pt", padding=True).to(device)
        prompt_embeds = self.mllm(**enc_inputs1, output_hidden_states=True).hidden_states[-1]
        
        
        prompt_attention_mask = enc_inputs1['attention_mask'].to(device)     
    ...
    def _apply_chat_template(self, prompt: str, images:PIL.Image.Image):
        prompts = [
            {
        "role": "user",
        "content": [
            ...
            {"type": "text", "text": "describ the input image in detail."},
        ],
        }
        ]
        image_inputs, video_inputs = process_vision_info(prompts)
        prompts = self.processor.apply_chat_template(prompts, tokenize=False, add_generation_prompt=True)
        return prompts, image_inputs

    def _apply_chat_template_negative(self, prompt: str):
        prompt = [
            {
                "role": "system",
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt = [self._apply_chat_template_negative(_negative_prompt) for _negative_prompt in negative_prompt]
    ...
    def __call__(
        ...
        output_images = []
        #for i in range(3): #, prompt in enumerate(prompts):
        ...
        if not return_dict:
            ...
            return FMPipelineOutput(images=output_images)
    ...
    def processing(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #print(text_guidance_scale, image_guidance_scale)
        ...
        del latents, model_pred, model_pred_ref, model_pred_uncond 
        return image
