...
class OmniGen2Pipeline(DiffusionPipeline, OmniGen2LoraLoaderMixin):
    ...
    def _get_qwen2_prompt_embeds(
        ...
        #     return_tensors="pt",
        # )
        enc_inputs = self.processor(images=images, text=prompt,
                                    return_tensors="pt", padding=True).to(device)
        prompt_embeds = self.mllm(**enc_inputs, output_hidden_states=True).hidden_states[-1]
        prompt_attention_mask = enc_inputs['attention_mask'].to(device)     
