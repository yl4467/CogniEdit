...
class Transport:
    ...
    def training_losses(
        ...
        if "cond" in model_kwargs:
            ...
            #xt = [th.cat([x, cond], dim=1) if cond is not None else x for x, cond in zip(xt, conds)]
            xt = th.cat([xt, conds], dim=1)
        #print(xt)  
        with model.cache_context("cond"):  
            model_output = model(hidden_states=xt, timestep=t, **model_kwargs)
        #print(model_output)
        B = len(x0)
        #print('test')
        ...
        if self.model_type == ModelType.VELOCITY:
            if isinstance(x1, (list, tuple)):
                ...
                terms["task_loss"] = mean_flat(((model_output[0][:, : ut.size(1)] - ut) ** 2))
        else:
            raise NotImplementedError

        terms["loss"] = terms["task_loss"]
        terms["t"] = t
        terms["last_t"] = saved_t
        return terms
